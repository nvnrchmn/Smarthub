...
const pengurusSeed = [
  {
    ...
    role: "Ketua_RT" as const,
    rumahIndex: 0,
    hub: "Kepala_Keluarga" as const,
    phone: "081200000001",
  },
  {
    nik: "3273012345670002",
    nama: "Siti Aminah",
    email: "sekretaris@smarthub.local",
  ...
  },
  {
    ...
    nama: "Dewi Kartika",
    email: "bendahara@smarthub.local",
...
const main = async (): Promise<void> => {
  ...
  await runWithTenant({ id_tenant: tenant.id_tenant }, async () => {
    ...
    for (const kategori of kategoriSeed) {
      kategoriList.push(
        await prisma.kategoriKeuangan.create({ data: { ...kategori, ...tenantCreateScope() } }),
      );
    }
    ...
    const ketua = await prisma.akunPengguna.findUnique({
